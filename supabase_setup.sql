...
create policy "Users can purchase/enroll." on public.enrollments for insert with check (auth.uid() = user_id);
...
create policy "Admins can view all results." on public.quiz_results for select using (
    exists (select 1 from public.profiles where id = auth.uid() and role = 'admin')
);
create policy "Users can record test scores." on public.quiz_results for insert with check (auth.uid() = user_id);
